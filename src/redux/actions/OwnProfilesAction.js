  ...
  api
    ...
    .then((res) => {
      ...
      if (payload) {
        ...
        payload?.length > 0 &&
          localStorage.setItem("ownProfile", JSON.stringify(payload?.at(-1)));
        let sortPayload = payload.sort((a, b) =>
          a.id > b.id ? 1 : b.id > a.id ? -1 : 0,
        );
        localStorage.setItem("ownProfiles", JSON.stringify(sortPayload));
        if(payload?.length === 0){
          localStorage.setItem('isProfileEmpty', true)
        }else{
          localStorage.setItem('isProfileEmpty', false)
        }
      } else {
